...
complexd* createq(int n) {
    ...
    for (i = 0; i < leng; i++) {
        seed = omp_get_wtime() + i;
        sign = 1;
        if ((rand_r(&seed) / (float) RAND_MAX) < 0.5) sign = -1;
        seed = omp_get_wtime() + i + sign;
        A[i].real(sign * (rand_r(&seed) / (float) RAND_MAX));
        seed = omp_get_wtime() + i + (rand_r(&seed) / (float) RAND_MAX);
...
int main(int argc, char **argv) {
    ...
    ofstream vect;
    results.open ("vector.txt", ios::out | ios::app);
    unsigned long long leng = 1 << n;
